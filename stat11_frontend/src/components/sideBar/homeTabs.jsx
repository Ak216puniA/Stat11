...
function HomeTabs() {
  ...
  const homeTabs =
    homeBarTabs.length > 0
      ? homeBarTabs.map((tab) => {
          const nologinTabCondition =
            authState.isAuthenticated && tab["text"] === "Login";
          const nologoutTabCondition =
            !authState.isAuthenticated && tab["text"] === "Logout";
          if (nologinTabCondition || nologoutTabCondition) return;

          const buttonBackgroundColor =
            sideBarState.currentTab === tab["text"]
              ? "primary.light"
              : "background.paper";
